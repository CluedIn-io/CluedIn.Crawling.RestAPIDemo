...
{
    public class GetCrawlJobDataBehaviour : RestProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(RestConstants.KeyName.Url, nameof(RestCrawlJobData.Url))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task RestCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Rest.Core.RestCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
