...
{
    public class GetCrawlJobDataBehaviour : ExampleRestProviderTest
    {
        ...
        [Theory]
        [InlineAutoData]
        public async Task ExampleRestCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.ExampleRest.Core.ExampleRestCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
